                                        ...
                                        @foreach ($lokasis as $index => $lokasi)
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex">
                                                        <!-- Edit Button -->
                                                        <a href="{{ route('admin.lokasi.edit', $lokasi->id) }}" class="btn btn-primary shadow btn-xs sharp me-1">
                                                            <i class="fas fa-pencil-alt"></i>
                                                        </a>
                                        
                                                        <!-- Delete Button -->
                                                        <form action="{{ route('admin.lokasi.destroy', $lokasi->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus lokasi ini?');">
                                                            @csrf
                                                            @method('DELETE')
